#!/usr/bin/env bash
# Golden-path transparent-proxy push via Gitea.
# Pushes a valid commit, waits for the API-approval flow, then re-pushes.
set -euo pipefail
...
git commit -m "feat: golden-path Gitea transparent-proxy test"
...
APPROVE_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    -d '{"user":"test-script","comment":"auto-approved by gitea/proxy-pass.sh"}')
