...
#   bash test/run-postgres.sh --no-teardown # leave the environment running afterwards
set -euo pipefail
...
echo ""
echo "========================================================"
echo "  Running test suite (PostgreSQL backend)"
echo "========================================================"
...
echo ""
echo "==> Verifying push records exist in PostgreSQL..."
...
if [ "$COUNT" -eq 0 ]; then
    echo "ERROR: No push records found — DB write or Flyway migration may have failed."
    exit 1
fi
