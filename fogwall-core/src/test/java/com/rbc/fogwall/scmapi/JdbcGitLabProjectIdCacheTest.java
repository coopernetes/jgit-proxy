...
/** Integration tests for {@link JdbcGitLabProjectIdCache} backed by an H2 in-memory database. */
class JdbcGitLabProjectIdCacheTest {

    JdbcGitLabProjectIdCache cache;

    @BeforeEach
    void setUp() {
        DataSource ds = DataSourceFactory.h2InMemory("node-id-cache-test-" + UUID.randomUUID());
        new JdbcPushStore(ds).initialize();
        cache = new JdbcGitLabProjectIdCache(ds, Duration.ofMinutes(5));
    }
    ...
    void store_expiredEntry_returnsEmpty() {
        ...
        new JdbcPushStore(ds).initialize();
        JdbcGitLabProjectIdCache expiredCache = new JdbcGitLabProjectIdCache(ds, Duration.ZERO);
