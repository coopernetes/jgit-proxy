#!/usr/bin/env bash
# Test script: pushes that SHOULD FAIL validation hooks via store-and-forward
...
run_test() {
    ...
    local repo_dir
    repo_dir=$(mktemp -d /tmp/push-test-fail-XXXX)
    cd /tmp
    git clone "${PUSH_URL}" "${repo_dir}" 2>&1
    cd "${repo_dir}"

    local branch="test/push-fail-$(date +%s%N | tail -c 8)"
