...
const NON_VALIDATION_STEPS = new Set([
  ...
  'AllowApprovedPushFilter', 'PushStoreAuditFilter', 'AuditLogFilter', 'ValidationSummaryFilter',
]);
...
function stepDisplayName(name: string): string {
  return STEP_DISPLAY_NAMES[name] ?? name.replace(/Filter$|Hook$/, '').replace(/([A-Z])/g, ' $1').trim();
}
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      {record && !loading && (
        <>
          ...
          <div className="bg-white rounded-lg shadow border border-gray-200 px-6 py-4">
            <div className="flex items-start gap-4">
              ...
              <div className="text-right text-sm text-gray-500 shrink-0">
                ...
                <div className="text-xs font-mono text-gray-400 mt-0.5">{record.commitTo ?? ''}</div>
            ...
            {record.blockedMessage && (
              <div className="mt-3 flex gap-2 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                <span>⚠️</span><span>{record.blockedMessage}</span>
              </div>
            )}
            {record.attestation && (
              ...
              <div className="space-y-2">
                {record.commits.map(c => (
                  <div key={c.sha} className="border border-gray-100 rounded p-3 space-y-1">
                    ...
                    {c.committerName && (c.committerName !== c.authorName || c.committerEmail !== c.authorEmail) && (
                      <div className="text-xs text-gray-500">
                    ...
                    {c.signedOffBy && c.signedOffBy.length > 0 && (
                      <div className="text-xs text-gray-500">
                        ...
                        {c.signedOffBy.map(sob => (
                          <span key={sob} className="ml-1 bg-green-50 text-green-700 border border-green-200 rounded px-1">{sob}</span>
                        ))}
                      </div>
                    )}
                  </div>
                ...
                {validationSteps.map(s => {
                  ...
                  return (
                    <div key={s.id}>
                      ...
                      >
                        <span className={s.status === 'PASS' ? 'text-green-500' : isFailed ? 'text-red-500' : 'text-gray-400'}>
                          {s.status === 'PASS' ? '✓' : isFailed ? '✗' : '–'}
                        </span>
                        <span className="text-sm text-gray-700 w-56 shrink-0">{stepDisplayName(s.stepName)}</span>
                        <span className="text-gray-500 text-xs truncate flex-1">
                          {s.errorMessage ?? s.blockedMessage ?? ''}
                        </span>
                        {isFailed && s.content && (
                          <span className="text-xs text-gray-400 shrink-0">{isOpen ? '▲ hide' : '▼ details'}</span>
                        )}
                      </div>
                      {isOpen && s.content && (
          ...
          {/* Diff */}
          <div className="bg-white rounded-lg shadow border border-gray-200 px-6 py-4">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Diff</h2>
            {hasDiff
          ...
          {record.status === 'BLOCKED' && (
            <div className="bg-white rounded-lg shadow border border-gray-200 px-6 py-5">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Review</h2>
